...
{
    public class GetPaginatedAidDistributionsByStatusQueryValidation
        : AbstractValidator<GetPaginatedAidDistributionsByStatusQuery>
    {
        public GetPaginatedAidDistributionsByStatusQueryValidation()
        {
        ...
        {
            RuleFor(ad => ad.Status)
                .NotNull().WithMessage(ad => $"{nameof(ad.Status)} can not be null.")
                .NotEmpty().WithMessage(ad => $"{nameof(ad.Status)} can not be empty.")
                .IsInEnum().WithMessage(ad => $"Invalid {nameof(ad.Status)} value. Allowed values are Done (1), InProgress (2), PostPoned (3).");
