...
namespace Charity.Services.ServicesImplementation
{
    public class AuthServices : IUnitOfServices
    {
        ...
        public async Task<AuthModel> GetTokenAsync(CharityUser user)
        {
        ...
        private async Task<string> GenerateTokenAsync(CharityUser user, Func<CharityUser, Task<List<Claim>>> Claims)
        {
        ...
        private async Task<List<Claim>> GetClaimsAsync(CharityUser user)
        {
            var userRolesNames = await _unitOfWork.CharityUsers.UserManager.GetRolesAsync(user);
            var userClaims = await _unitOfWork.CharityUsers.UserManager.GetClaimsAsync(user);

            var userRoles = (await _unitOfWork.CharityRoles.GetAllAsync(r => userRolesNames.Contains(r.Name ?? "Not Found"))).ToList();

            var permissions = new List<Claim>();
            foreach (var role in userRoles)
            {
                var roleClams = await _unitOfWork.CharityRoles.RoleManager.GetClaimsAsync(role);
        ...
        public async Task<AuthModel> GetRefreshTokenAsync(CharityUser user)
        {
        ...
        public async Task<string> GenerateVerificationCodeAsync(CharityUser user)
        {
            var code = await _unitOfWork.CharityUsers.UserManager.GenerateTwoFactorTokenAsync(user, TokenOptions.DefaultEmailProvider);
            return code;
        }
        ...
        public async Task<bool> VerifyCodeAsync(CharityUser user, string code)
        {
            var isValid = await _unitOfWork.CharityUsers.UserManager.VerifyTwoFactorTokenAsync(user, TokenOptions.DefaultEmailProvider, code);
            return isValid;
        }
        ...
        public async Task<EmailConfirmationResponse> ConfirmEmailAsync(EmailConfirmationRequest emailRequest)
        {
            ...
            var user = await _unitOfWork.CharityUsers.UserManager.FindByIdAsync(emailRequest.UserId);
            if (user == null)
            {
            ...
            var result = await _unitOfWork.CharityUsers.UserManager.ConfirmEmailAsync(user, emailRequest.Token);
            if (result.Succeeded)
            {
                emailResponse.IsConfirmed = true;
